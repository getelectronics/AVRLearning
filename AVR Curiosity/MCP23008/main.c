...
#define EXPANDER_I2C_ADDR 0x25  // MCP23008 Address: 0x25
#define SSD1306_I2C_ADDR  0x3D  // SSD1306 OLED Address
...
void Expander_WriteRegister(uint8_t reg, uint8_t value) {
    if (TWI0_Start((EXPANDER_I2C_ADDR << 1) | 0)) {
        TWI0_Write(reg);
        TWI0_Write(value);
        TWI0_Stop();
    }
...
void Expander_SetOutputs(uint8_t mask) {
...
void Mode_BinaryCounter(void) {
    ...
    for (uint16_t count = 0; count <= 255; count += 5) {
        Expander_SetOutputs((uint8_t)count);
        PORTF.OUTTGL = HEARTBEAT_LED_MASK;
        _delay_ms(50);
    }
}
...
int main(void) {
    ...
    SSD1306_DrawString("EXPANDER 0X25", 2, 20);
