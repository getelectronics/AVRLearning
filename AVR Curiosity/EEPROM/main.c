...
#define EEPROM_BASE_ADDR 0x54  // AT24CM02 Block 0 Address
...
uint8_t EEPROM_WritePage(uint32_t memAddr, const uint8_t *data, uint8_t len) {
    // Calculate device address byte with A16 bit (bit 1 of I2C address)
    uint8_t deviceAddr = EEPROM_BASE_ADDR | ((memAddr >> 16) & 0x01);

    // Transmit Device Address (Write Mode)
    if (!TWI0_Start((deviceAddr << 1) | 0)) return 0;

    // Send 16-bit word address (MSB then LSB)
    ...
    // Delay 10ms for internal EEPROM write cycle to complete
    _delay_ms(10);
    return 1;
}
...
uint8_t EEPROM_ReadBuffer(uint32_t memAddr, uint8_t *buffer, uint8_t len) {
    ...
    // 1. Send dummy write frame to position internal address pointer
    if (!TWI0_Start((deviceAddr << 1) | 0)) return 0;
    if (!TWI0_Write((uint8_t)(memAddr >> 8))) return 0;
    if (!TWI0_Write((uint8_t)(memAddr & 0xFF))) return 0;

    // 2. Restart and read data bytes
    if (!TWI0_Start((deviceAddr << 1) | 1)) return 0;

    for (uint8_t i = 0; i < len - 1; i++) {
        buffer[i] = TWI0_Read(1); // ACK for intermediate bytes
    }
    buffer[len - 1] = TWI0_Read(0); // NACK & STOP on final byte
...
int main(void) {
    ...
    SSD1306_DrawString("AT24CM02 0X54", 2, 22);

    // Test string to write and read back
    const uint8_t writeMsg[] = "NANO EEPROM OK";
    uint8_t readMsg[16];

    // Write message to memory address 0x00000
    if (EEPROM_WritePage(0x00000, writeMsg, sizeof(writeMsg))) {
        SSD1306_DrawString("WRITE: SUCCESS", 4, 10);
    } else {
        SSD1306_DrawString("WRITE: FAILED ", 4, 10);
    }

    _delay_ms(100);

    // Read message back from memory address 0x00000
    if (EEPROM_ReadBuffer(0x00000, readMsg, sizeof(writeMsg))) {
        SSD1306_DrawString("READ: ", 6, 10);
        SSD1306_DrawString((char*)readMsg, 6, 46);
    } else {
        SSD1306_DrawString("READ:  FAILED ", 6, 10);
    }
